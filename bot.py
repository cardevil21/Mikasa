...
# ==================== DATABASE ====================
conn = sqlite3.connect('/app/data/data.db', check_same_thread=False)
c = conn.cursor()
c.executescript('''
...
def handle_cmd(m):
    ...
    if cmd == '/start': send(chat, '🎌 Mikasa Ackerman protecting this group!\n/help for commands.')
    elif cmd == '/help': send(chat, '<b>Commands</b>\n\n/help /rules\n\n<b>Admins:</b>\n/warn /resetwarn /warnings\n/mute /unmute /kick /ban\n/purge /settings /toggle')
    elif cmd == '/rules': send(chat, '📜 <b>GROUP RULES</b>\n\n1. Anime only\n2. No promo/scam\n3. No other channel/website\n4. Only animethic.in/.xyz\n5. No hate speech\n6. Be friendly\n\n⚠️ 3 warns = 10min mute')
...
def process(u):
    if 'message' in u:
        ...
        if get_setting(cid, 'anti_flood') != 'off' and is_flood(uid):
            delete(cid, m['message_id']); mute_user(cid, uid, 5); send(cid, '🌊 Flood! Muted 5 min.'); add_report('s'); return
        if get_setting(cid, 'anti_spam') != 'off' and is_scam(txt):
            delete(cid, m['message_id']); handle_violation(cid, uid, m['from']['first_name'], 'Scam detected'); return
        ...
        if cm.get('new_chat_member', {}).get('status') == 'member':
            ...
            add_report('j')
            if is_raid(cid): send(ADMIN, f'🚨 Raid Alert! Chat: {cid}')
